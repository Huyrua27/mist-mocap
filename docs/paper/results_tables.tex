...
\begin{table}[t]
\centering
\caption{Recovering a \emph{drifting} inter-camera offset (mean $|\hat\delta(t)-\delta(t)|$
in frames; lower better). A single lag (CC-const) cannot track drift; Caspi--Irani is a
coarse affine grid; CC-slide degrades on the short windows drift tracking needs.
ContinuSyncFormer in short sliding windows with a robust line fit (CSF-slide) wins at
every drift level and is the only method that improves as drift grows.}
\label{tab:b3_drift}
\begin{tabular}{rrrrr}
\toprule
Drift (fr) & CC-const & Caspi--Irani & CC-slide & \textbf{CSF-slide (ours)} \\
\midrule
0.0 & 0.186 & 0.313 & 0.173 & \textbf{0.153} \\
0.5 & 0.230 & 0.300 & 0.194 & \textbf{0.133} \\
1.0 & 0.320 & 0.310 & 0.186 & \textbf{0.123} \\
2.0 & 0.593 & 0.255 & 0.283 & \textbf{0.099} \\
\bottomrule
\end{tabular}
\end{table}
